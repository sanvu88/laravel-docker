...
Documentation=man:php-fpm7.4(7)
...
Type=notify
ExecStart=/usr/sbin/php-fpm7.4 --nodaemonize --fpm-config /etc/php/7.4/fpm/php-fpm.conf
ExecStartPost=-/usr/lib/php/php-fpm-socket-helper install /run/php/php-fpm.sock /etc/php/7.4/fpm/pool.d/www.conf 80
ExecStopPost=-/usr/lib/php/php-fpm-socket-helper remove /run/php/php-fpm.sock /etc/php/7.4/fpm/pool.d/www.conf 80
ExecReload=/bin/kill -USR2 \$MAINPID
LimitNOFILE=65535
LimitMEMLOCK=infinity
PrivateTmp=true
...
cat >"/etc/php/7.4/fpm/conf.d/50-redis.ini" << EOF
extension=redis.so
EOF

cat >"/etc/php/7.4/cli/conf.d/50-redis.ini" << EOF
extension=redis.so
EOF
